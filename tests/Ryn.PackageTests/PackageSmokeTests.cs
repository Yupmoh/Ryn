...
public sealed class PackageSmokeTests : IDisposable
{
    ...
    [Fact]
    public async Task NuGet_consumer_builds_and_generators_each_emit_one_router()
    {
        ...
        // Ryn + a plugin is the supported app shape AND the case that regressed: each source
        // generator must come through exactly once (from its corresponding runtime package).
#pragma warning disable CA2007 // xUnit manages SynchronizationContext
        ...
        [
            ...
            Path.Combine(solutionRoot, "src", "Ryn.Callbacks.Generator", "Ryn.Callbacks.Generator.csproj"),
            Path.Combine(solutionRoot, "src", "Ryn.Core", "Ryn.Core.csproj"),
            Path.Combine(solutionRoot, "src", "Ryn.Ipc", "Ryn.Ipc.csproj"),
            Path.Combine(solutionRoot, "src", "Ryn.Callbacks", "Ryn.Callbacks.csproj"),
        ...
        // Build it. If either generator were shipped twice, this fails with duplicate generated
        // members; if either did not flow at all, its generated registration method would not resolve.
        var buildResult = await RunDotnetAsync(appDir, "build", "-c", "Release");
        buildResult.ExitCode.Should().Be(0, $"dotnet build failed:\n{buildResult.Output}");

        // Each source generator produced exactly one router.
        ...
        generatedFiles.Should().HaveCount(2,
            "the IPC and callback generators should each emit one router (extra files indicate duplicate analyzer distribution)");

        var routerContents = new List<string>(generatedFiles.Length);
        foreach (var generatedFile in generatedFiles)
            routerContents.Add(await File.ReadAllTextAsync(generatedFile));

        routerContents.Should().ContainSingle(
            content => content.Contains("ICommandRouter", StringComparison.Ordinal)
                       && content.Contains("greet", StringComparison.Ordinal),
            "the IPC generator should emit one command router");
        routerContents.Should().ContainSingle(
            content => content.Contains("IRynCallbackRouter", StringComparison.Ordinal),
            "the callback generator should emit one callback router");
...
#pragma warning restore CA2007
    ...
    private static void WriteTempApp(string appDir, string feedDir, string version)
    {
        ...
        // NavigationCallbacks.cs — a [RynCallback] method so the callback generator has work to do
        File.WriteAllText(
            Path.Combine(appDir, "NavigationCallbacks.cs"),
            """
            using Ryn.Callbacks;
            using Ryn.Core;
