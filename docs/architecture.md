...
| JS <> C# bridge   | Notification, Audio,         |
|                    | Tray, Updater                |
+-------------------+------------------------------+
| Ryn.Core                                         |
...
invoke('app.greet', {name:'W'})
  ...
  +-- XHR POST /ipc/cmd/{id}/app.greet
      body: {"name":"W"}
                    ...
                    Parse URL: /ipc/cmd/1/app.greet
                    ReadRequestBody() -> {"name":"W"}
                    AcceptEmptyResponse() [immediate 200]
                              |
                    DispatchCommandAsync(1, "app.greet", body)
                              |                            Task.Run() -->
                              |                              |
                              ...
                              |                              |
                              |                        router.CanRoute("app.greet") -> true
                              |                              |
                              |                        router.RouteAsync()
                              |                              |
...
{
    public bool CanRoute(string command) => command is "app.greet" or "app.add";
...
**ryn.json present** = deny by default (only listed commands are allowed). **No ryn.json** is build-dependent: a **Debug** build falls back to allow-all (dev convenience), while a **Release** build **fails closed (deny-all)** and emits a one-time startup warning so a mis-deployed app is never silently wide open. See [SECURITY.md](../SECURITY.md) for the full model.
...
{
    if (!_enforced) return;   // allow-all mode (e.g. Debug build with no ryn.json); Release with no ryn.json is enforced + deny-all
...
`scope` and `scopedCommands` in `ryn.json` restrict plugin-level configuration:

- `scope` clamps the plugin's allowed filesystem paths to the listed directories (glob-capable)
- `scopedCommands` clamps the shell plugin to specific binaries **and the arguments they may receive** (Tauri-style argv templates), e.g. `[{ "name": "git", "args": ["status"] }]`. A bare `commands` array (binary names only, any arguments) is still accepted as a legacy form, but `scopedCommands` is preferred.

These restrictions are applied at the plugin options level -- `RynCapabilities.GetScope()` returns the scope constraints (`AllowedPaths`, `CommandScopes`, `AllowedSchemes`), and plugin registration code intersects them with programmatic options.
